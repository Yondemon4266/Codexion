...
/*   Updated: 2026/04/29 17:11:40 by aluslu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../data.h"

static void	set_wait_cooldown_time(long long l_ready, long long r_ready,
	long long *wait_time, long long current_time)
{
...
int	handle_cooldown(t_coder *coder)
{
	...
	if (coder->left_dongle != coder->right_dongle)
	{
		...
		r_ready = coder->right_dongle->released_time
	...
	if (current_time == -1)
		return (ERROR);
	set_wait_cooldown_time(l_ready, r_ready, &wait_time, current_time);
	if (wait_time > 0)
